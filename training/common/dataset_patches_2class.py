...
class DDSM(torch.utils.data.Dataset):
    def __init__(self, root, image_list_path, transform, augmentation):
        ...
        self.suspicious_indices = [i for i, item in enumerate(self.images) if item[1] == 1]
    ...
    def pick_new_normal_images(self):
        self.shuffled_indices = self.suspicious_indices + \
                                sample(self.normal_indices, len(self.suspicious_indices))
